...
import 'package:omspos/utils/custom_log.dart';
import 'package:provider/provider.dart';
// Import your Esewa class
// import 'path/to/your/esewa.dart';
...
class _BookingBottomSheetState extends State<BookingBottomSheet> {
  ...
  Future<void> _processBooking(Map<String, dynamic> formData) async {
    try {
  ...
  Future<void> _handleEsewaPayment(Map<String, dynamic> formData) async {
    try {
      // Create Esewa instance and initiate payment
      Esewa esewa = Esewa();

      // You might want to modify your Esewa class to accept callback for success
      // For now, we'll call the pay method directly
      await esewa.initPaymentWithCallback(
        onSuccess: () async {
          // Payment successful, now create the booking
          await _processBooking(formData);
        },
        onFailure: () {
          setState(() {
            _isProcessing = false;
                              ...
                              try {
                                ...
                                final Map<String, dynamic> formData = {
                                  ...
                                  'move_in_date': formatDate(
                                      formValues['move_in_date'] as DateTime),
                                  'move_out_date':
                                      formValues['move_out_date'] != null
                                          ? formatDate(
                                              formValues['move_out_date']
                                                  as DateTime)
                                          : null,
                                  'monthly_rent': monthlyRent,
                                  'security_deposit': securityDeposit,
                                  'profession':
                                      formValues['profession'] as String,
                                  'peoples': peoples,
                                  'room_id': widget.room.roomId,
                                  'tenant_id': userId,
                                  'landlord_id': landlordId,
                                ...
                                final paymentMethod =
                                    formValues['payment_method'] as String?;

                                if (paymentMethod == 'esewa') {
                                  // Launch eSewa payment
                                  await _handleEsewaPayment(formData);
